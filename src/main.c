...
static void speech_kill(struct vc_data *vc)
{
	...
	if (val == 2 || spk_killed) {
		/* dead */
		spk_shut_up &= ~0x40;
		synth_printf("%s\n", speakup_msgs[IAM_ALIVE]);
	} else {
		synth_printf("%s\n", speakup_msgs[YOU_KILLED_SPEAKUP]);
		spk_shut_up |= 0x40;
	}
}
...
static void speakup_off(struct vc_data *vc)
{
	if (spk_shut_up & 0x80) {
		spk_shut_up &= 0x7f;
		synth_printf("%s\n", speakup_msgs[HEY_THATS_BETTER]);
	} else {
		spk_shut_up |= 0x80;
		synth_printf("%s\n", speakup_msgs[YOU_TURNED_ME_OFF]);
	}
	speakup_date(vc);
}
...
static void say_attributes(struct vc_data *vc)
{
	...
	int bg = spk_attr >> 4;
	if (fg > 8) {
		synth_printf("%s", speakup_msgs[BRIGHT]);
		fg -= 8;
	}
	synth_printf("%s", colors[fg]);
	if (bg > 7) {
		synth_printf("%s", speakup_msgs[ON_BLINKING]);
		bg -= 8;
	} else
...
static void toggle_cursoring(struct vc_data *vc)
{
	...
	if (++cursor_track >= CT_Max)
		cursor_track = 0;
	synth_printf("%s\n", speakup_msgs[CURSOR_MSGS_START + cursor_track]);
}
...
speakup_win_set(struct vc_data *vc)
{
	char info[40];
	if (win_start > 1) {
		synth_printf("%s\n", speakup_msgs[WINDOW_ALREADY_SET]);
		return;
	}
	if (spk_x < win_left || spk_y < win_top) {
		synth_printf("%s\n", speakup_msgs[END_BEFORE_START]);
		return;
	}
...
speakup_win_enable(struct vc_data *vc)
{
	...
	win_enabled ^= 1;
	if (win_enabled)
		synth_printf("%s\n", speakup_msgs[WINDOW_SILENCED]);
	else
		synth_printf("%s\n", speakup_msgs[WINDOW_SILENCE_DISABLED]);
}
