...
static void synth_flush(void)
{
	while ((inb(speakup_info->synth_port_tts + UART_LSR) & BOTH_EMPTY) != BOTH_EMPTY);
	outb(SYNTH_CLEAR, speakup_info->synth_port_tts);
	spk_serial_out(PROCSPEECH);
}
...
static int serprobe(int index)
{
	...
	spk_serial_release();
	timeouts = speakup_info->synth_alive = 0; /* not ignoring */
	return -1;
}
